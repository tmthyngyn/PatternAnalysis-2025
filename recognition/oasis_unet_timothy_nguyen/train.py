...
def build_model(num_classes: int, device: torch.device) -> nn.Module:
    """
    Defensive model builder that supports either modules.UNet or modules.UNet2D.
    Prefers explicit in/out args, falls back to default ctor if needed.
    """
...
def dice_per_class(pred_logits: torch.Tensor, target: torch.Tensor, num_classes: int) -> torch.Tensor:
    """
    Computes per-class Dice for a batch.
    pred_logits: (B, C, H, W)
    target:      (B, H, W) int64 in [0..C-1]
    returns:     (C,) mean over batch
    """
...
def parse_args():
    ...
    p.add_argument("--save-dir", type=str, default="./trained_models/oasis_unet")
    p.add_argument("--num-workers", type=int, default=2)
    p.add_argument("--no-class-weights", action="store_true", help="Disable inverse-frequency class weights")
    return p.parse_args()
...
def main():
    ...
    # Datasets (will raise helpful FileNotFoundError if layout is wrong)
    try:
    ...
    if args.no_class_weights:  # this line would be invalid; fix variable name
        pass
    # Compute class weights unless disabled
